...
class Minimist {
  ...
  Map<String, dynamic> anonymousParse (){
    ...
    var parsedArgumentsList = arguments.map<dynamic>((argument) {
      var _parsedArgumentsList = argumentPattern
                                .allMatches(argument)
                                .toList();
    ...
    for (var i=0; i<parsedArgumentsList.length; i++) {
      if (
        parsedArgumentsList[i].runtimeType.toString() == 
        '_RegExpMatch'
      ) {
        ...
        if (matched.group(1) == '--') {
          ...
          if (matched.group(5) != null) {
            ...
            if (matched.group(3) != null) {
              ...
              if (i < parsedArgumentsList.length - 1) {
                if (parsedArgumentsList[i+1].runtimeType.toString() != '_RegExpMatch') {
          ...
          var parsedParameters = RegExp(r'([a-zA-Z])')
                                          .allMatches(matched.group(2))
                                          .map((match) {
                                            return match.group(1);
                                          })
                                          .toList();
          var allExceptLastParams = parsedParameters
                                    .sublist(0,parsedParameters.length - 1);
